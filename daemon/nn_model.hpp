...
public:
    ...
    bool run(const std::string& dns_query)
    {
        ...
        {
            std::cout << "Wrong in/out parameters" << std::endl;
            return false;
        }
        ...
        std::cout << "Digit ratio: " << float_inputs[2] << ", shannon_entropy: " << float_inputs[1] << ", length: " << float_inputs[0] << ", uppercase_ratio: " << float_inputs[3] << std::endl;
        ...
        std::cout << "in_scale: " << input_scale << ", in_zp: " << input_zp << std::endl;
        ...
        std::cout << "out_scale: " << output_scale << ", out_zp: " << output_zp << std::endl;
        ...
        if (ret < 0) { printf("rknn_set_io_mem for input failed!\n"); return -1; }

        ret = rknn_set_io_mem(ctx, output_mem, &output_attr);
        if (ret < 0) { printf("rknn_set_io_mem for output failed!\n"); return -1; }
        ...
        printf("\n--- Array-Stride Compliant Quantization ---\n");
        for (int i = 0; i < 4; ++i) {
            ...
            printf("Feature [%d] placed at byte offset [%d] -> INT8: %d\n", 
                    i, i * element_stride, static_cast<int8_t>(rounded));
        }
        printf("Model natively expects input format (fmt): %d\n", input_attr.fmt);
        ...
        float logit_score = (float)(signed_raw - signed_zp) * output_attr.scale;

        float real_probability = 1.0f / (1.0f + exp(-logit_score));

        int final_class = (real_probability >= 0.5f) ? 1 : 0;

        printf("Raw INT8: %d | Logit: %f | Probability: %f | Class: %d\n", 
...
private:
    bool checkInOutParameters()
    {
        ...
        {
            std::cout << "Expected " << DNS_CLASSIFIER_INPUTS_COUNT << " inputs, got: " << io_num.n_input << std::endl;
            return false;
        }
        if (io_num.n_output != DNS_CLASSIFIER_OUTPUTS_COUNT)
        {
            std::cout << "Expected " << DNS_CLASSIFIER_OUTPUTS_COUNT << " output, got: " << io_num.n_output << std::endl;
            return false;
        }
        return true;
    }
